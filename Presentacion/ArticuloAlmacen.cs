...
{
    public partial class ArticuloAlmacen : Form
    {
        ...
        private void ArticuloAlmacen_Load(object sender, EventArgs e)
        {
        ...
        public void ListarArt(int idEmpresa)
        {
            ...
            tblAlmacen = obj.GridArt(idEmpresa, ref blnTodoOK);
        ...
        public void ListarArtAcc()
        {
            ...
            if (txtIdAlm.Text.Trim() != string.Empty)
            {
                ...
                tblArtAcc = obj.ArtAccesoAlm(idAlmacen, ref blnTodoOK);
                ...
                foreach (DataRow row1 in tblArtAcc.Rows)
                {
                    ...
                    //asignas el dato a cada columna de la row
                    row["id"] = row1["intIdArtAlm"].ToString();
        ...
        public void Insertar()
        {
            try
            {
                using (TransactionScope tscTrans = new TransactionScope())
                {
                    ...
                    objNgAlm.InsertarArtAlmacen(idAlmacen, idEmpresa, tabla, ref blnTodoOK);
        ...
        {
            src2.Filter = string.Format("nombre LIKE '%" + txtBuscar2.Text + "%' OR codigo LIKE '%" + txtBuscar2.Text + "%'");
        ...
        private void btnPasaTodo_Click(object sender, EventArgs e)
        {
            ...
            {
                foreach (DataGridViewRow row in dgvListaArt.Rows)
                {
                    int intIdArt = Convert.ToInt32(row.Cells["id_Art"].Value);
                    string strCoArt = row.Cells["codigo_Art"].Value.ToString();
                    string strNoArt = row.Cells["nombre_Art"].Value.ToString();
                    tabla.Rows.Add(new object[] { 0, strCoArt, strNoArt, intIdArt });
                }
                return;
            }
            ...
            else
            {
                foreach (DataGridViewRow row in dgvListaArt.Rows)
                {
                    ...
                    string strCoArt = row.Cells["codigo_Art"].Value.ToString();
                    string strNoArt = row.Cells["nombre_Art"].Value.ToString();
                    string ss = row.Cells["id_Art"].Value.ToString();
