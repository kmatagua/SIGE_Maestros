...
{
    public partial class MaquinaUniNeg : Form
    {
        ...
        DataTable tblUniNeg = new DataTable();
        BindingSource src = new BindingSource();

        public MaquinaUniNeg()
        {
            InitializeComponent();
        ...
        private void MaquinaUniNeg_Load(object sender, EventArgs e)
        {
            cbxUniNeg.SelectedIndexChanged -= cbxAlmacen_SelectedIndexChanged;
        ...
        public void ListarMaq(int idEmpresa)
        {
            ...
            tblUniNeg = obj.GridMaq(idEmpresa, ref blnTodoOK);
            foreach (DataRow row1 in tblUniNeg.Rows)
            {
                //creas una nueva row
        ...
        public void ListarMaqAcc(int idEmpresa)
        {
            ...
            {
                NGMaquina obj = new NGMaquina(Configuracion.Sistema.CadenaConexion);
                int idUniNeg = (cbxUniNeg.SelectedValue is DBNull) ? 0 : Convert.ToInt32(cbxUniNeg.SelectedValue.ToString());
                tblMaqAcc = obj.MaqAccesoUniNeg(idUniNeg, idEmpresa, ref blnTodoOK);
        ...
        public void ListarComboUniNeg()
        {
            bool blnTodoOK = false;
            Clases.InicialCls.LeerXml();

            NGComun obj = new NGComun(Configuracion.Sistema.CadenaConexion);
            //tblUniNeg = obj.ComboUniNeg(idEmpresa, ref blnTodoOK);
            tblUniNeg = obj.Listar_Combo2("UNIDAD_NEGOCIO", 0, idEmpresa, idUsuario, 0, 0, ref blnTodoOK);
            cbxUniNeg.DataSource = tblUniNeg;
            cbxUniNeg.DisplayMember = "strNoUndNeg";
            cbxUniNeg.ValueMember = "intIdUndNeg";
        ...
        private void btnPasa_Click(object sender, EventArgs e)
        {
            ...
            if (Convert.ToInt32(cbxUniNeg.SelectedValue) <= 0)
            {
                MessageBox.Show("Selecione una Unidad de Negocio", this.Text, MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
        ...
        public void Insertar()
        {
            try
            {
                using (TransactionScope tscTrans = new TransactionScope())
                {
                    ...
                    NGUndNegocio objNg = new NGUndNegocio(Configuracion.Sistema.CadenaConexion);
                    int idUniNeg = (cbxUniNeg.SelectedValue is DBNull) ? 0 : Convert.ToInt32(cbxUniNeg.SelectedValue.ToString());
                    objNg.InsertarMaqUniNeg(idUniNeg, tabla, ref blnTodoOK);
        ...
        {
            int valor = (cbxUniNeg.SelectedValue is DBNull) ? 0 : Convert.ToInt32(cbxUniNeg.SelectedValue.ToString());
